...
import ColumnRelationCell from '../ColumnRelationCell/ColumnRelationCell';
...
const SchemaTable = () => {
  ...
  const [columnRelations, setColumnRelations] = useState<ColumnRelation[]>([]);
  const [lineageColumnMap, setLineageColumnMap] = useState<
    Record<string, { fqn: string; function?: string }[]>
  >({});
  ...
  const fetchColumnRelations = useCallback(async () => {
    if (!tableFqn) {
      return;
    }
    try {
      const data = await getColumnRelations(tableFqn);
      setColumnRelations(data);
    } catch {
      setColumnRelations([]);
    }
  }, [tableFqn]);

  const fetchLineageColumns = useCallback(async () => {
    if (!tableFqn) {
      return;
    }
    try {
      const data = await getLineageDataByFQN({
        fqn: tableFqn,
      ...
      const colMap: Record<string, { fqn: string; function?: string }[]> = {};
      const allEdges = [
      ...
      for (const edge of allEdges) {
        for (const colLineage of edge.columns ?? []) {
          if (colLineage.toColumn) {
            for (const fromCol of colLineage.fromColumns ?? []) {
              const fromParts: string[] = fromCol.split('.');
              const fromColName = fromParts.pop() ?? '';
              const fromTableFqn = fromParts.join('.');

              const toParts: string[] = colLineage.toColumn.split('.');
              const toColName = toParts.pop() ?? '';
              const toTableFqn = toParts.join('.');

              // For columns belonging to the current table, record where they map to
              if (fromTableFqn === tableFqn) {
                if (!colMap[fromColName]) colMap[fromColName] = [];
                colMap[fromColName].push({
                  fqn: colLineage.toColumn,
                  function: colLineage.function,
                });
              }
              if (toTableFqn === tableFqn) {
                if (!colMap[toColName]) colMap[toColName] = [];
                colMap[toColName].push({
                  fqn: fromCol,
                  function: colLineage.function,
                });
              }
  ...
  const columns: ColumnsType<Column> = useMemo(
    () => [
      ...
      },
      {
        ...
        dataIndex: 'columnRelation',
        key: 'columnRelation',
        width: 220,
        render: (_: unknown, record: Column) => (
          <ColumnRelationCell
            canEdit={editTagsPermission}
