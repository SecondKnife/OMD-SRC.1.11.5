...
const ExplorePageV1: FC<unknown> = () => {
  ...
  const searchIndex = useMemo(() => {
    ...
    if (searchHitCounts && isNil(tabInfo)) {
      ...
      return activeKey ?? (SearchIndex.TABLE as unknown as ExploreSearchIndex);
    }
  ...
  const effectiveSearchResults: SearchResponse<ExploreSearchIndex> | undefined =
    useMemo(() => {
      if (isTourOpen) {
        return mockSearchData as unknown as SearchResponse<ExploreSearchIndex>;
      }

      // In normal mode, rely solely on real search results.
      // If there are no results, return undefined instead of mock sample_data.
      return searchResults;
    }, [isTourOpen, searchResults]);
