...
export const getTableDetailPageBaseTabs = ({
  ...
  return [
    ...
    },
    {
      label: (
        <TabsLabel
          id={EntityTabs.COLUMN_RELATION}
          name={get(
            labelMap,
            EntityTabs.COLUMN_RELATION,
            t('label.column-relation')
          )}
        />
      ),
      key: EntityTabs.COLUMN_RELATION,
      children: (
        <iframe
          src={`${CustomRoutes.COLUMN_RELATION}?tableFqn=${encodeURIComponent(tableDetails?.fullyQualifiedName ?? '')}`}
          title="column-relation"
          style={{ width: '100%', height: 'calc(100vh - 260px)', border: 'none' }}
        />
      ),
    },
    {
